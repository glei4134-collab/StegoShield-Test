...
def embed_dct(image_bytes: bytes, secret_bytes: bytes, quality: int = 75) -> bytes:
    ...
    """
    if HAS_CV2:
        return _embed_dct_cv2(image_bytes, secret_bytes, quality)
    else:
...
def _embed_dct_cv2(image_bytes: bytes, secret_bytes: bytes, quality: int = 75) -> bytes:
    ...
    if total_bits > h * w * 3:
        raise InvalidInputError(f'数据太大，超出图片容量')
    ...
    buf = io.BytesIO()
    cv2.imwrite(buf, bgr, [cv2.IMWRITE_JPEG_QUALITY, quality])
    return buf.getvalue()
...
def extract_dct(jpeg_bytes: bytes) -> bytes:
    ...
    Args:
        jpeg_bytes: JPEG 图片字节
    
    Returns:
        原始 payload 字节
    """
    if HAS_CV2:
        return _extract_dct_cv2(jpeg_bytes)
    else:
